...
if AquaShine.OperatingSystem == "iOS" then
	...
	-- Based on https://chaosinmotion.blog/2012/03/26/how-do-i-know-what-environment-variables-are-on-ios/
	...
	AquaShine.Log("Temp dir i", temp_dir)
elseif AquaShine.OperatingSystem == "Windows" then
	...
	AquaShine.Log("Temp dir w", temp_dir)
elseif AquaShine.OperatingSystem == "Android" then
	...
	AquaShine.Log("Temp dir a", temp_dir)
else	-- Linux & Mac OS X
	...
	if os.execute("[ -d \""..temp_dir.."\" ]") ~= 0 then
		temp_dir = "/tmp"
		AquaShine.Log("Temp dir lx", temp_dir)
	end
end
