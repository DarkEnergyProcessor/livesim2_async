-- Modified version of 30log https://github.com/Yonaba/30log
-- changed "name" to "__name" for the class name
local next         = next
local assert       = assert
local pairs        = pairs
local type         = type
...
local function extend(self, name, extra_params)
	...
	heir.__name    = extra_params and extra_params.__name or name
	heir.__index = heir
	heir.super   = self
	heir.mixins = {}
...
baseMt = {
	...
	__tostring = function(self,...)
		if _instances[self] then
			return ("instance of '%s' (%s)"):format(rawget(self.class,'__name') 
								or '?', _instances[self])
		end
		return _classes[self] 
							and ("class '%s' (%s)"):format(rawget(self,'__name')
							or '?',
