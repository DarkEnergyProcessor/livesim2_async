...
-- This file should be very well-documented
...
-- LOVE's RNG is based on 64-bit Xorshift from http://xorshift.di.unimi.it
-- See LOVE's RandomGenerator.cpp in modules/math for more details
local randomState = love.math.newRandomGenerator()

local function randomNote(notes, includeExclude)
	if includeExclude then
		return notes[randomState:random(1, #notes)]
	else
		local notesIncluded = {}
		...
		return notesIncluded[randomState:random(1, #notesIncluded)]
	end
end
...
local function randomizer3(m_notesList, timeRestriction)
	...
	for _, v in ipairs(m_notesList) do
...
return function(x, seedlo, seedhi)
	local a = {}
	...
	randomState:setSeed(seedlo or 0, seedhi or 0)
