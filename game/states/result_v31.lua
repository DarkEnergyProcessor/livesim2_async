...
function resultScreen:start(arg)
	self.persist.nameText = love.graphics.newText(self.data.mainFont, arg.summary.name)
...
function resultScreen:update(dt)
	...
	if needRefresh then
		...
		self.persist.valueInfoText:addf(
			{accuracyColorMap[1][1], PV(info.perfect, interp)},
			120, "center",
			-- 214 + 0 * 120
			214, 368
		)
		-- GREAT
		self.persist.valueInfoText:addf(
			{accuracyColorMap[2][1], PV(info.great, ip)},
			120, "center",
			-- 214 + 1 * 120
			334, 368
		)
		-- GOOD
		self.persist.valueInfoText:addf(
			{accuracyColorMap[3][1], PV(info.good, ip)},
			120, "center",
			-- 214 + 2 * 120
			454, 368
		)
		-- BAD
		self.persist.valueInfoText:addf(
			{accuracyColorMap[4][1], PV(info.bad, ip)},
			120, "center",
			-- 214 + 3 * 120
			574, 368
		)
		-- MISS
		self.persist.valueInfoText:addf(
			{accuracyColorMap[5][1], PV(info.miss, ip)},
			120, "center",
			-- 214 + 4 * 120
			694, 368
		)
