...
local Imagescale
local FirstTrans = {imageopacity = 0, textpos = 0, textopacity = 1}
local FirstTransTween = tween.new(233, FirstTrans, {imageopacity = 1, textpos = 480})
local TextAura = {textpos = 480, opacity = 0.5}
...
function CoverArt.Draw()
	...
	setColor(1, 1, 1, FirstTrans.textopacity)
	...
	if FirstTransComplete and not(TextAuraComplete) then
		...
		setColor(1, 1, 1, TextAura.opacity)
	...
	setColor(1, 1, 1, FirstTrans.imageopacity)
